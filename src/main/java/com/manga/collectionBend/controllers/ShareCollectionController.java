...
public class ShareCollectionController {
    ...
    public ApiResponse<List<SharedCollectionDto>> getSharedWithMe(@AuthenticationPrincipal UserEntity currentUser) {
        return ApiResponse.success(shareService.getSharedWithMe(currentUser.getUserId()));
    }

    @PatchMapping("/{shareId}/viewed")
    public ApiResponse<String> markViewed(@PathVariable Integer shareId) {
        shareService.markAsViewed(shareId);
        return ApiResponse.success("Marked as viewed");
    }

    @PatchMapping("/{shareId}/action")
    public ApiResponse<String> updateAction(@PathVariable Integer shareId, @RequestParam ShareActionStatus status) {
        shareService.updateActionStatus(shareId, status);
